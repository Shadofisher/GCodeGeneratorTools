...
int main(int argc, char * argv[])
{
    ...
    start_index = findIndex("0.00","0.00",num_paths,&reverse);
    ...
    if (reverse)
    {
        append_gcode_reverse(start_index,argv[2]);
    }else
    {
        append_gcode(start_index,argv[2]);
    }
...
//    printf("\n\rClosest to origin: %d",start_index);
    ...
    while((paths_stil_valid(num_paths)) > 0)
    {
        if (reverse)
        {
            start_index = findIndex(GC_Paths[index].x_start,GC_Paths[index].y_start,num_paths,&reverse);
        }else
        {
            start_index = findIndex(GC_Paths[index].x_end,GC_Paths[index].y_end,num_paths,&reverse);
        }
        printf("\n\rReverse: %d",reverse);
        if (reverse)
        {
